...
| `length [mm]` | トラック端点間の長さで絞り込み。下限を ~80 mm にすれば**宇宙線の貫通トラック**だけ、上限を ~30 mm にすれば**短いアルファ線**だけ残る。再フィットなしで live 反映 |

操作部と 3D 図の間に**長さ分布のヒストグラム**を出す。アルファ + 宇宙線の 2 山構造があれば短い (数 mm) と長い (~drift cage 長) の山にギャップが見えるはずなので、`length` slider を調整する目安になる。スライダで絞った範囲は緑帯でハイライト。
